...
const App = {
    ...
    bindManglishToggle() {
        ...
        const toolbarBtn = document.getElementById('toolbar-manglish');
        const toggleFn = () => {
            this.manglishMode = !this.manglishMode;
            if (this.manglishMode) {
                btn.classList.add('active');
                btn.textContent = '✓ Manglish ON';
                if (toolbarBtn) { toolbarBtn.style.background = '#dbeafe'; toolbarBtn.style.color = '#1e40af'; }
                ...
                if (toolbarBtn) { toolbarBtn.style.background = '#eef2ff'; toolbarBtn.style.color = '#4338ca'; }
        ...
        if (toolbarBtn) toolbarBtn.addEventListener('click', toggleFn);
    },
    ...
    openTeleprompter(text) {
        ...
        win.document.write('<input type="range" min="1" max="50" value="50" id="speed-slider">');
        ...
        win.document.write('  var s=parseInt(slider.value)||50;');
